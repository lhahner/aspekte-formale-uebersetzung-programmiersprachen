...
Weiterhin ist PL/I eine Altsprache, die seit den 1960er Jahren im Einsatz ist und durch den Generationenwechsel an Entwicklern verliert. Wartung und Entwicklung werden so häufig schwer und teuer.\footcite[Vgl. ][S. 227ff.]{histopli} 
...
Das Ergebnis des Frontends ist eine Zwischencodedarstellung. Diese Zwischencodedarstellung wird an das Backend übergeben, um daraus Maschinencode zu generieren. Der Maschinencode kann dann auf dem zugrundeliegenden System ausgeführt werden. \footcite[Vgl. ][S.106ff. ]{aho}
...
Abbilding \ref{fig:transpiler} zeigt die Prozessschritte eine Transpilers.
...
Sowohl One-Pass-Compiler, Transpiler bzw. Source-to-source Compiler als auch herkömmliche Compiler übersetzen das Programm basierend auf seiner Gesamtstruktur. \footcite[Vgl. ][S. 18ff. ]{assambly}
...
Shell ist keine Programmier- oder Skriptsprache. Hingegen bringen die genannten Interpreter, Erweiterungen mit, die einer Programmiersprache ähneln. Bei \verb+bash+ werden etwa Programmiersprachen-ähnliche Konstrukte, wie Variablen, Kontrollstrukturen oder Funktionen verwendet. \footcite[Vgl. ]{gnubash}

In der Linux Manual Page wird \verb+bash+ auch als Command-language Interpreter bezeichnet. \footcite[Vgl. ][]{bashman}
...
Um den Unterschied zwischen einem Interpreter, wie bash, und einem Compiler zu verdeutlichen, wird in Abbildung \ref{fig:shell}  die Arbeitsweise des Interpreters von bash dargestellt.
...
Schließlich wird der des ausgeführten Skripts durch die \verb+stdio+-Bibliothek ausgegeben, in der Kommandozeile ausgegeben. \footcite[Vgl. ][]{gnubash}
...
Wie schon in den Abbildungen \ref{fig:transpiler} dargestellt sind die ersten Arbeitsschritte des Transpilers die lexiklaische und syntaktische Analyse.
...
Eine formale Sprache unterscheidet sich von einer natürlichen Sprache in der Anwendungsform. Die Anwendung von solchen Sprachen dient der logisch präzisen Beschreibung von Ausdrücken. \footcite[Vgl. ][S. 149ff. ]{automata} 
Eine Grammatik besteht aus Variablen, Terminalsymbolen, einen Startsymbol und einer Produktion. Zusammengefasst in:
...
Grammatiken lassen sich weiter durch die Chomsky Hierarchie spezifizieren. Noam Chomsky unterteilt Grammatiken in vier Ebenen. Ebene null beschreibt unbeschränkte-, Ebene eins kontextsensitve-, Ebene zwei kontextfreie- und Ebene drei reguläre Grammatiken.\footcite[Vgl. ][]{chomsky}
...
Die Darstellung der Grammatik erfolgt in der Regeln in einer Art \ac{ebnf}. 
...
Im Körper der Methode wird der If-Ausdruck weiter definiert. Terminalsymbole werden in der JavaCC Grammatik mit den größer-als und kleiner-als Zeichne umrandet. Die Nicht-Terminalsymbole hingegen sind wie im \ref{lst:ifstatement} dargestellt weitere Methoden, die wiederum weitere Ausdrücke beschreiben.
...
Ein weiterer Vorteil den Maven liefert ist die vordefinierte Projektstruktur. Mavens Projektstruktur liefert zwei gespiegelte \verb+src+ Ordner, welche je den Quellcode enthalten und die dazugehörigen Tests.
Auch hier finden sich Benutzer und Administratoren leichter zurecht.
...
Die Architektur des Transpiler wurde nachdem Vorbild eines \ac{mvc} Konzept entworfen. 
...
Der Benutzer oder der Administrator kann mit der Ausgewählten Schnittstelle interagieren und so den Transpiler benutzen. Eine Schnittstelle, ist entweder über die ac{ide} in der das Projekt importiert wird oder die Kommandozeile.
...
In Abbildung \ref{fig:modules} sind die Module in dem Ablauf des Transpilers dargestellt.
...
Abbidlung \ref{fig:modulesdep} aktuell die Beziehungen der Module untereinander, ohne dabei detailierter auf Klassenhierachien einzugehen. 
...
% Wie sind die Module momentan gebaut?
Der Aufbau der Module ist dabei durch Klassen spezifiziert. Jede Klasse bietet eine Sammlung von Methoden zur Lösung eines Problems,
...
die erneut Ressource bereitstellen.
...
\paragraph{Parser}
Das Parser-Modul deckt somit die lexikalische und die syntaktische Analyse des PL/I-Quellcodes ab.
Wie schon in Kapitel 1.4 erwähnt werden jegliche Klassen des Parsers durch die \verb+.jjt+ Grammatikdatei 
generiert. Da diese Klassen sehr umfangreich sind, werden diese in Abbildung \ref{fig:moduleparser} lediglich in Abgekürzter Form dargestellt. 
...
In Abbildung \ref{fig:symboltable} ist das Modul \verb+symboltable+ als UML dargestellt.
...
Wenn ein Bezeichner in die Symboltabelle eingefügt wird müssen, ein String für den Bezeichner, ein String für den Typ, ein Integer für die Sichtbarkeit und ein Integer für die Hierarchiestufe der Variable hinterlegt sein. Nur so können andere Module wie der Parser, die Werte korrekt verarbeiten.
Der Parser verwendet die Symboltabelle etwa dazu, diese zu durchsuchen und um festzustellen ob der Wert bereits eingefügt wurde.
 		...
 		if (startNode.jjtGetParent() == null) {
 			if (this.hasChildren(startNode)) {
 				if(AstMapper.astMapper.get(startNode.getId()) != null) {
 					var.setTranslationBevaior(AstMapper.astMapper
 					.get(startNode.getId()));
 			...
 			for (int i = 0; i < startNode.jjtGetNumChildren(); i++) {
 				if(AstMapper.astMapper.get(startNode.jjtGetChild(i).getId()) != null) {
 					var.setTranslationBevaior(AstMapper.astMapper
 					.get(startNode.jjtGetChild(i).getId()));
...
%\dhgefigure[h]{{mapper-module-uml.drawio.png}{width=16cm, height=11cm}{Mapper Modul}{fig:mapper}
...
Eine Ver\"anderung der Laufzeit, wird mit Ausdruck in Listing ?.? und Zeile ?.? nicht erzeugt.
