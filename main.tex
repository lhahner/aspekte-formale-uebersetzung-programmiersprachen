...
Sollte in Java eine äquivalente Abbruchbedingung erzeugt werden, wurde in der aktuellen Version des Transpilers das Einfügen einer do-while-Schleife gewählt. Der Schleifen-Körper der do-while-Schleife beinhaltet die gleiche Logik, die auch in Listing \ref{lst:untilwhile} im Schleifen-Körper dargestellt ist. Die eigentliche Until-Bedingung wird dann negiert, um eine zusätzliche Abbruchbedingung zu definieren. Diese Lösung ist in Listing \ref{lst:javauntilwhile} dargestellt.
...
In Listing \ref{lst:pliwrite} ist ein \verb+WRITE+ Befehl gelistet.
...
\begin{lstlisting}[language=PL/I, caption=Scanner Objekt in Java, label={lst:pliwrite}]
	WRITE FILE (file_1) FROM (var_1);
\end{lstlisting} 

Die Übersetzung des PL/I-Quellcode führt zu der Ausgabe in Listing \ref{lst:javawriter}.

\begin{lstlisting}[language=Java, caption=Writer, label={lst:javawriter}]
...
Um die in Kapitel 3.2 beschriebenen Übersetzungen zu Testen wurden für die entscheiden Übersetzungsmethoden,
Unit-Tests geschrieben.
Allgemein erfolgt die Integration der Transformation von weiteren PL/I-Ausdrücken durch Test-Driven-Development Vorgehensweise.
Zuerst wurde die Übersetzungsentscheidung ausgearbeitet und ein dazugehöriger Unit-Test geschrieben,
woraufhin die Logik implementiert wurde.
Beispielhaft ist in Listing \ref{lst:javaunittest} eine Test-Methode für das Mappen von Variablen dargestellt.
...
übersetzt wird wird.
Mit jeden Test wird auch ein Parser Objekt erzeugt, dadurch wird auch mit jeden Test der PL/I-Ausdruck gegen die geschriebene Grammatik überprüft.
...
Bei der Implementierung des Tests, wurden sowohl negativ, wie auch postive Tests geschrieben.
In Listing \ref{lst:javaunittest} handelt es sich um einen Positiven Test. 
Der dazugehörige negative Test, überprüft die Übersetzung eines nicht zulässigen Ausdruck mit einem Fehlerhaften Beispiel.

So wurde die Allgemeine Veränderung des PL/I-Codes geprüft und integriert.
Eine weitere Veränderung die der Parser versuchen kann, hängt mit der Laufzeit der Programms zusammen.
In dem nächsten Kapitel wird die Performance von dem PL/I-Quellcode und dem Java-Zielcode verglichen.
