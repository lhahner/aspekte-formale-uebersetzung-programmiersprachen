...
Ab hier verarbeitet nun das Betriebssystem die Befehle des Shell-Skriptes und führt das entsprechende Programm mithilfe der Pfade zur vorkompiliertem Binärdatei aus. Dieser Prozess ist vage vergleichbar mit dem Einsetzen von Registeradressen während der Kompilierung.
...
% 
In diesem Kapitel wird das Vorgehen der syntaktische und lexikalische Analyse vorgestellt.
...
\dhgefigure[h]{parser-generation.png}[scale=0.7]{Ein- und Ausgabeverarbeitungsprozess des Compiler-Compiler}{fig:ausgabe}[][]
\pagebreak
...
Die Darstellung der Grammatik erfolgt in einer Art \ac{ebnf}. 
...
\dhgefigure[h]{parser-module-uml.drawio.png}[width=160mm,height=130mm]{UML des Parser-Moduls}{fig:moduleparser}[][]
\pagebreak
...
wenn dieser schon vorhanden ist und den selben Sichtbarkeitswert hat. Dabei wird das Modul \verb+SymbolTable+ implementiert und mit der Methode \verb+insertId+ ein Bezeichner in die Symboltabelle eingefügt. Im folgenden Unterkapitel wird das Modul der Symboltabelle genauer beschrieben. 
...
In diesem wurden Kind \newline -Klassen der Exception Klasse der Java Standard Bibliothek implementiert.
...
\begin{minipage}[b]{0.48\linewidth}
	...
	\begin{lstlisting}[frame=single, numbers=left, mathescape,%
		caption={Annotations Lösung}, label={lst:annotationdecimal},
...
\dhgefigure[h]{mainframe-system-info.png}[scale=0.47]{Mainframe System Spezifikation}{fig:mainframesysteminfo}[][]
...
Damit wird das Programm auf der aktuell (Juli 2024) neusten Hardware von IBM kompiliert und ausgeführt.
...
ausgeführt werden. Das Batchjob-Programm wurde in der Job-Control-Language (JCL) geschrieben. Dem  PL/I-Programm werden nach jedem Testlauf eine größere Primzahlen übergeben. Der erst Testlauf behandelt die Primzahl 2,147,483,647, welche die 1772 von Leonhard Euler größte entdeckte Primzahl mit 10 Stellen ist. Die nächste Primzahl ist eine sogenannte einzigartige Primzahl mit 12 Stellen. Daraufhin folgt die bisher von Menschen, ohne Beihilfe eines Computers größte bestimmte Primzahl 67.280.421.310.721. Die letzte getestete Primzahl konnte bisher nicht ohne Hilfe eines Computers belegt werden.\footcite[Vgl. ][]{prime} In Tabelle \ref{tab:plibenchmark} sind alle Primzahlen mit ihrer Laufzeit zu sehen. 
...
Die Benchmark-Zahlen aus Tabelle \ref{tab:plibenchmark} zeigen das der Mainframe für die Berechnung der Primzahlen maximal eine Sekunde braucht. Nun soll dieser Benchmark mit dem des Java-Programms verglichen werden. Zum Vergleich wird das während des Integration-Test übersetzte Programm aus Anhang \ref{lst:javaprobedivision} ebenfalls für die Berechnung der Primzahlen getestet. Die Hardware-Spezifikationen des Systems, auf dem der Integrations-Test durchgeführt wird. Sind in Tabelle \ref{tab:hardwartable} beschrieben.
...
Für die Berechnung der Primzahlen brauchte, bis auf die von Leonhard Euler bestimmte Primzahl alle mehr als eine Sekunde. Bei der Beurteilung sollten die Hardware Unterschiede zwischen dem IBM-Mainframe und dem Linux-Computer berücksichtigt werden.

Während des Benchmarks wurde ebenfalls versucht, die Primzahl 170,141,183,460,469,231,- \newline 731,687,303,715,884,105,727
zu berechnen. Dabei musste der Java-Zielcode angepasst werden, da diese Zahl die maximale Anzahl an Ziffern übersteigt, die ein primitiver Datentyp in Java verarbeiten kann. Daher wurde stattdessen der nicht-primitive Datentyp \verb+BigDecimal+ verwendet. Im Anhang \ref{lst:javaprobedivisionang} befindet sich das angepasste Programm, um solche großen Zahlen zu berechnen, muss der Transpiler bzw. die Übersetzung angepasst werden, da in Java die Behandlung solcher großen Zahlen nur mit dem nicht-primitiven Datentyp \verb+BigDecimal+ oder \verb+BigInt+ möglich ist.
...
Zusammenfassend wird so eine schwäche des Transpilers deutlich. Durch die Überschreitung von Grenzstellen, sollte der Transpiler entsprechend eine andere Typisierung wählen. In der aktuellen Version ist, die differenzierte Typisierung je nach Wertzuweisung noch nicht implementiert. Der Entwickler muss, in diesem Fall das Übersetzte Programm selbst anpassen. In dem folgenden Kapitel sollen weitere schwächen betrachtet und diese Arbeit evaluiert werden.
\pagebreak
\section{Fazit}
% Erreichte Entwicklung
Auch nach dieser Arbeit wurde der Transpiler zwar erweitert, aber noch nicht fertiggestellt.
Das entwickelte Programm bietet lediglich die grundlegenden Voraussetzungen für die Übersetzung von PL/I-Programmen.
...
Damit der Transpiler auch tatsächlich für Juniore PL/I-Entwickler zum testen von Quellcode auf einem Desktop Computer verwendet werden kann, müssen weitere Ausdrücke in die Grammatik eingearbeitet werden.
Erst wenn nahezu alle aktuell verwendeten PL/I-Ausdrücke auch von dem Parser verarbeitet werden kann, kann auch dieser Transpiler verwendet werden um PL/I-Programme auf einem bspw. Linux-Desktop Computer auszuführen.
Es ist dabei auch weiterhin fraglich, ob die Übersetzung in Java dieselbe Laufzeit simuliert, wie in PL/I gewünscht. Um dies sicherzugehen sollten neben Unit-Tests, Integrations-Tests und System-Tests auch User-Acceptence-Tests berücksichtigt werden.
...
Der nächste Schritte der Entwicklung wird die Erweiterung des Negativ-Testing sein, zwar wurden in der aktuellen Version solche berücksichtigt. Jedoch entstehen immer wieder bei der Übersetzung Fehlermeldungen, die einen eigentlich korrekten Ausdruck bemängeln.
Nachdem die Anwendung ausführlich getestet wurde, wird die in dieser Arbeit vorgestellte Version veröffentlicht. Daraufhin soll die Grammatik verbessert und die PL/I-Sprachreferenz nahezu vollständig eingearbeitet werden.
...
Zusammenfassend war diese Arbeit erneut sehr lehrreich, jedoch auch sehr aufwendig. Sie zeigt weitere Fortschritte und entsprechend Erweiterungspotential.
