...
Daraufhin ist es möglich, das Maven-Projekt in Eclipse zu importieren. Unter der Registerkarte "Datei" gibt es die Option, ein Maven-Projekt in Eclipse zu importieren. Dabei erscheint das Menüfenster aus Abbildung \ref{fig:importproject}.

\dhgefigure[h]{import-maven-project.png}[scale=0.5]{Importieren eines Maven Projekts}{fig:importproject}
\pagebreak

Im nächsten Menüfenster sollte der Ordner ausgewählt werden, in dem die pom.xml liegt. Ist der Ordner korrekt ausgewählt, wird das Projekt in Eclipse importiert. Nun kann das Projekt verwendet werden. Das Verzeichnis \verb+src/main/java/res/pli+ ist das Standard-Eingabeverzeichnis. Hier kann eine PL/I-Datei abgelegt werden, die übersetzt werden soll.

Es ist auch möglich, das Standard-Eingabeverzeichnis zu ändern. Im Ordner \verb+src/main/java/res/config+ befindet sich die Konfigurationsdatei \verb+config.properties+. In dieser Datei kann die Variable \verb+PATH+ geändert werden, um PL/I-Quellcode aus einer anderen Projektstruktur zu transformieren.
...
In den nachfolgenden Kapiteln werden PL/I-Strukturen beschrieben und die Transformation in Java diskutiert. Außerdem sollen die Ergebnisse der Projektarbeit IV revidiert und erneut diskutiert werden.
...
In PL/I können Variablen, die als \verb+DECIMAL+ deklariert wurden, miteinander verrechnet werden. Dadurch sind einfache bis hin zu komplexen arithmetischen Operationen möglich. 
...
Ist nun in der Klasse \verb+DECIMAL+ für jede Arithmetische Operation eine Methode definiert, die es ermöglichen \verb+DECIMAL+-Objekte miteinander zu verrechnen, wäre die Übersetzung in Listing \ref{lst:javapliarithmeticexpression} äquivalent zu Listing \ref{lst:pliarithmeticexpression}.
...
\begin{lstlisting}[language=PL/I, caption=Deklaration einer Varibale des Typs Decimal, label={lst:annotationdecimal}]
	@Decimal(10)
	double var_1;
...
\begin{lstlisting}[language=Java, caption=Annotation Lösung um eine Prozedur zu übersetzen, label={lst:javaannotationproc}]
@Char(5)
...
Soll dann etwa einer der Parameter beim aufrufen der Methode Zurückgegeben werden, kann dies einfach über das Casten zu dem entsprechenden Rückgabetyp erreicht werden.
...
Würden hier erneut nicht-Primitive Typen wie das \verb+DECIMAL+ Objekt verwendet, wäre eine solche einfache Vergleichs Operation nicht möglich.
...
Um nun neben einem einfachen Programmablauf, zusätzlicher Logik eines Programms auch Ein- und Ausgabe Operationen von dem PL/I-Quellcode in den Java-Zielcode übersetzen zu können, wird in dem nachfolgenden Kapitel sowohl die Konsolen, also die Datei Ein- und Ausgabe diskutiert.
...
Damit ist das lesen eines Eingabe Strings möglich.

Gepaart mit dem Programmfluss, Verzweigungen und schleifen ist die Transformation von einfachen PL/I-Programmen denkbar.
Etwa kann ein einfaches Login-Programm mit den transformierten PL/I-Strukturen übersetzt werden. 

Damit das übersetze Programm auch auf das Dateisystem zugreifen kann, wird ebenfalls die Ein- und Ausgabe von Dateien integriert.
...
In Kapitel 1.1 wurde bereits erwähnt das PL/I Programm hauptsächlich auf einem Mainframe-System ausgeführt werden.
...
Wird nun Listing \ref{lst:plifiletyp} dem Transpiler als Eingabe übergeben entsteht der Java-Zielcode in Listing \ref{lst:plifiletyp}.

\begin{lstlisting}[language=Java, caption=File Objekt in Java, label={lst:javafiletpye}]
public File file_1 = new File("");
\end{lstlisting} 

Hierbei ist anzumerken das bisher lediglich ein File Objekt instanziiert wurde.
Es wurde kein Datei-Pfad übergeben und somit auch keine Datei eingelesen.
Allgemein ist Fraglich wie das einlesen von Dateien mit dem Transformierten PL/I Programm erfolgen wird, da die Organisation von Dateien auf einem z/Os-System anders aufgeteilt ist wie die eine Linux oder Windows Desktop Computer und eine Kompatibilität mit der Zeichencodierung von etwa einer vsam Datei ebenfalls einer Transformation bedarf.
...
Wird in einem PL/I Programm die definierte Datei gelesen und in einer Variable gespeichert, geschieht das in PL/I mit dem \verb+READ+-Statement.
...
\begin{lstlisting}[language=Java, caption=Scanner Objekt in Java, label={lst:javafiletpye}]
