...
Für die unterschiedlichen Benutzergruppen, werden unterschiedliche Verwendungsmöglichkeiten des Transpilers angeboten.
...
\paragraph{Eclipse}
Wie eingangs beschrieben kann die \ac{ide} Eclipse dazu verwendet werden den Transpiler zu benutzen. 
...
Es wurden in der Projektarbeit-IV selbstgeschriebene nicht-Primitive Datentypen verwendet um die PL/I Datentypen, in entsprechenden Java-Zielcode zu übersetzen. Dies ist für den Umfang von Deklarationen ausreichend.
Hingegen werden erst mit dem Umgang dieser Datentypen in einer Programmroutine, Mängel des Designs sichtbar.
Ein Beispiel hierfür ist die DECIMAL Klasse. 

Gegenwärtig wurde der DECIMAL-Datentyp aus PL/I, in Java als Nicht-primitiver Datentyp über die Klasse DECIMAL implementiert.
So wurde etwa über den Konstruktor eine Längenbeschränkung in Java möglich.  

Werden nun in dem PL/I-Quellcode die Deklarierten Variablen verwendet um Mathematische Berechnungen zu erstellen, ist dies ohne Problem mit den Mathematischen Standard Operatoren wie Plus oder Minus möglich.

Um solche Ausdrücke auch in Java mit einem DECIMAL Objekt zu ermöglichen, muss die Klasse DECIMAL erweitert werden.
...
In Java ist es nicht möglich Operatoren zu Überladen wie etwa in C++, sodass die Arithmetischen Operationen auch mit nicht-primitiven Datentype wie etwa dem DECIMAL-Objekt möglich sind.

Eine solche Erweiterung, würde Zwangsläufig zu einer Komplexen Programmstruktur des übersetzen Java Programms führen.
Angenommen der PL/I-Ausdruck in Listing ?.? wird mithilfe des Transpilers in Java übersetzt.
...
Ist nun in der Klasse DECIMAL für jede Arithmetische Operation eine Methode definiert, die es ermöglichen DECIMAL Objekte miteinander zu verrechnen, wäre die Übersetzung äquivalent zu Listing ?.?.
...
In Listing ?.? wird deutlich das der Arithmetische Ausdruck durch die Verwendung von Methoden, anstatt der üblichen Operatoren weniger leserlich geworden ist.
Eine solche Komplexitätssteigerung kann sich auf die Qualität des Endergebnisses des Transpilers auswirken. Etwa durch die verschachtelung der Operator Methoden.

Somit ist eine Lösung nötig die einfache Arithmetische Ausdrücke zulässt und gleichzeitig eine Längenbeschränkung.

Um einfache Arithmetische Ausdrücke zu implementieren, kommen in Java lediglich die Verwendung von primitiven Datentypen wie \verb+double+, \verb+float+ oder \verb+int+ in Frage. In Verbindung mit einer Annotation ist in Java auch eine Längenbeschränkung denkbar.
Mithilfe der Jarkarta Validations Software-Bibiolthek kann eine eigene Annotation erzeugt werden, die mit einer Java-Reflection überprüft wird.
Dazu wird eine Implementierung von Valdierungs Klasse des übersetzen Programms notwendig. 
Jedoch kann mit dieser Lösung der Ausdruck in Listing ?.? in den Java-Ausdruck in Listing Transformiert werden.
...
Zu erkennen ist das mithilfe von primitvien Datentypen die gleiche Darstellung der Arithmetische Berechnung wie in Listing ?.? ermöglicht wird.
Dadurch ist auch die Einbindung und Erweiterung des übersetzen PL/I-Quellcode erleichtert.
...
Die beschriebene Version des Transpiler implementiert bisher lediglich die \verb+@Decimal+ Annotation.
Weshalb in den nachfolgenden Kapiteln weiterhin Klassen wie etwa die Klasse \verb+CHAR+ eingebunden werden. Außerdem sollen nun die weiteren Übersetzungsentscheidungen vorgestellt werden.
